...
class LaneNetPostProcessor(object):
    ...
    """
    def __init__(self, cfg, ipm_remap_file_path='./LaneNet/data/tusimple_ipm_remap.yml'):
        """
    ...
    def postprocess(self, binary_seg_result, instance_seg_result=None,
        ...
        for index, single_lane_pts in enumerate(src_lane_pts):
            ...
            step = int(math.floor((end_plot_y - start_plot_y) / 1)) #20201219
            lane_dict.append({})
            for plot_y in np.linspace(start_plot_y, end_plot_y, step):
                ...
                lane_dict[lane_count][int(interpolation_src_pt_y)] = int(interpolation_src_pt_x)

                lane_color = self._color_map[index].tolist()
                cv2.circle(source_image, (int(interpolation_src_pt_x),
                                          int(interpolation_src_pt_y)), 5, lane_color, -1)
            lane_dict[lane_count] = dict(sorted(lane_dict[lane_count].items()))
            lane_count += 1
        ret = {
